...
\textcolor{darkgreen}{Question: How does the 2 different breed of dog which belongs to same class pull together in the latent space eventhough loss fn explicitly doesn't do that (situation specific to our dataset will be: some specific single dose image and
some specific drug screened image can also have sililarity like they belong to same class
eventhough they have diffrent drug concentration applied ) Data augmentation helps here to generalise the features/characterics of different breed but in same class images while training.}
...
6. Supervised Simclr: Make sure no same breed images in the negative samples. Does this increase the confidence score for 1's in the cross entropy loss? 
