...
calculate cosine distance between each image (of all 1246 images that we used to train simclr ) and its corresponding augmentation that we used in simclr.
Idea is to ie check whether simlcr learned to 
be invariant to position/flipped error specifically for day 10 images or blured/sharpened or brightness in microscope error because of manual 
handling/transporting. If its learned well then we expect the cosine distance between the original image feat and its corresponding augmentatoin  feat should be same 
or very close to each other. specifically  features after projection head since the after projection head features are used for reducing loss function.
                                                                    

it also applicable to time prediction  ranking evaluation. in time prediction ranking strategy, we will predict from day 7 to day 10. since we
may have mispositioned images of day 10,the more simclr is learned to invariant to flips and rotation the better day 10 prediction.

setup: we calculate the difference between the cosine distance of original image and augmented image for each augmentatoin and then we average the difference for 
all images. if the average difference is close to 0 then it means simclr learned to be invariant to that specific augmentatoin. while augmentation values were random according to the speicifc 
data aug that we explained in chepter simclr.
...
Within the simclr feats strong have more avg difference comparing other aug avg errors for flip and rotation augs , this is due to the  strong intense croping of 
0.1 to 1 percentage of the image. since in those crops doesn't sometimes include the cancer round tissue model sometimes it leads to less learning of the overall structure invarience.
Surprisingly avg error are higher generally for after feats than beofore. strong data aug feats learned to be invariant for brightness than other augs as it have 
very low avg error differenece. original image vectors have far less and even closer to 0 avg errors comparing to simclr feats, this make this evaluation makes still
interesting to see the comparison performance of  original image vector vs simclr feats for different ranking strategy.
...
4. leads to the idea of ranking strategy for k means centroid approach. it should give different distance to single dose and exploded and closer distance to same gp from control.
\textcolor{red}{show original day 7to day 10 sd is smae as day 7 to day 7.}
