...
2. Since we train to classify these 2 groups, if we use softmax score as metric 
2. While inference we add explod images to see if they are overlapping with  control or single dose images when we take  softmax probability as metric for ranking. If there is no overlap that means it seperates control explod and single dose seperately we get 100 percentage accuracy as per our customized ranking accuracy calculator.
