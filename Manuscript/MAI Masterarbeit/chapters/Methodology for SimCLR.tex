...
\begin{enumerate}
 ...
 increase. It increaases the sharpness by a factor of 2. We set this also as our default for all data augmentation.
\end{enumerate}
...
crops without a cancer cell in it is train to have high cosine 
simililartiy to these same image smaller crop patch of dark cancer cell from center due to the design of  loss function in SimCLR. Which doesn't make sense. Hence 
the next data augmentation sequence designed to have random crop with scale 0.4 to 1 ( so that assuming crop patches will have atleast small portion of cancer cell)
and brightness and contrast variations changed to lower=1-0.2, upper=1+0.2 and lower=1-0.35, upper=1+0.35 consequently. I named this augmentation as 'sweet'.
which illustrated in \ref{fig:strong_aug} second row.
 ...
 small cancer cell surronding lot of debris around it. so if we apply the above random crop and resize aug: smaller crop from the round cancer cell part have high 
 cosine similarity with some other small crop from the round cancer cell part of the same image. which doesn't connect anything about the debris surrounded by for 
 exploed images. In general model will also learn to map high cosine similarity between dark area to gray area which can happen to generally all images except 
 single dose images.

 Maybe its better to not to crop and just resize to 96*96 all the time so that model learn to map high cosine similarity by seeing the whole picture. 



from day 7 to day 10, maybe its better to 




then I removed contrast. because if I tried to make 
...
Every data augmentations explored in nutshell in \ref{fig:strong_aug}
\begin{figure}[H]
  \centering
